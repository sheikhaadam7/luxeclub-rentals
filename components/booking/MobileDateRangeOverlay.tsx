...
export function MobileDateRangeOverlay({
  ...
  // Auto-scroll to the month containing the current `from` (or today) when opening.
  useEffect(() => {
    if (!open) return
    const t = setTimeout(() => {
      ...
      <div ref={bodyRef} className="flex-1 overflow-y-auto overscroll-contain px-4 py-4">
        <DayPicker
          ...
          classNames={{
            ...
            day: 'h-11 flex items-center justify-center',
            day_button:
              'w-11 h-11 text-sm font-bold rounded-full transition-colors text-zinc-900 active:bg-zinc-200',
