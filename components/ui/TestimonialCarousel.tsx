...
export function TestimonialCarousel({ reviews }: { reviews?: Testimonial[] }) {
  ...
  return (
    ...
    >
      {/* Arrows flanking the review card */}
      <div className="flex items-center justify-center gap-4 max-w-3xl mx-auto">
        <div className="hidden sm:block shrink-0">
          <ArrowButton direction="left" onClick={() => goTo((activeIndex - 1 + testimonials.length) % testimonials.length)} />
        </div>
        ...
        <div className="hidden sm:block shrink-0">
          <ArrowButton direction="right" onClick={() => goTo((activeIndex + 1) % testimonials.length)} />
        </div>
      </div>
